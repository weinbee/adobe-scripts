/**
 * This script is provided free of charge for VCT278 students. Hit me up at nhartmann@shoreline.edu with questions and I'll MAKE A NEW VERSION FOR
 */
...
panelCEV.spacing = 0;
panelCEV.margins = 0; // was 5, now 0
...
keyframeCountText.characters = 2;  // Adjust this value for your needs
    ...
    var panelThree = panelTwoThree.add("panel", undefined, "aNullment");
    panelThree.orientation = "row";
    var nullButton = panelThree.add("button", [10, 10, 90, 30], "Null");
    ...
    var sortButton = panelSort.add("button", [10, 10, 90, 30], "Sort");
    // UI Group - Easement
    ...
    // Slider for Easing
    var slider = sliderGroup.add("slider", [10, 10, 100, 30], 0, 0, 100);
    slider.value = 70;
    slider.stepdelta = 1;
    ...
    valueText.bounds = [105, 10, 125, 30];
    ...
    var button1 = buttonsGroup.add("button", [10, 10, 25, 25], "<");
    var button2 = buttonsGroup.add("button", [30, 10, 45, 25], "<>");
    var button3 = buttonsGroup.add("button", [50, 10, 65, 25], ">");

    // UI Group for Reverse Button
var panelReverse = panelThree.add("group"); // Add the reverse panel to panelThree (aNullment panel)
panelReverse.orientation = "row";

// Create the 'reverse' button
var reverseButton = panelReverse.add("button", [10, 10, 90, 30], "Reverse");
...
buttonC.onClick = function() {
    ...
    function reverseLayerOrder() {
        ...
        if (!(comp instanceof CompItem)) {
          alert('The selected item is not a composition. Please select a composition.');
          return;
        }
        ...
        if (len <= 1) {
          alert('Please select at least two layers to reverse order.');
          return;
        }
      
        for (var i = 0; i < len; i++) {
          selectedLayers[i].moveToEnd();
        }
