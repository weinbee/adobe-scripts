...
function unPrecompose() {
  ...
  if (activeItem != null && activeItem instanceof CompItem && activeItem.selectedLayers.length > 0) {
      app.beginUndoGroup("Duplicate-Cut-Paste Layer");

      var duplicateID = app.findMenuCommandId("Duplicate");
      if (app.executeCommand(duplicateID)) {
          var cutID = app.findMenuCommandId("Cut");
          // Select the last layer (which should be the duplicated one)
          activeItem.layer(activeItem.numLayers).selected = true;
          // Deselect all other layers
          for (var i = 1; i < activeItem.numLayers; i++) {
              activeItem.layer(i).selected = false;
          }
          if (app.executeCommand(cutID)) {
              var pasteID = app.findMenuCommandId("Paste");
              if (!app.executeCommand(pasteID)) {
                  alert("Failed to paste layer!");
              }
          } else {
              alert("Failed to cut layer!");
          }
      } else {
          alert("Failed to duplicate layer!");
      }
      app.endUndoGroup();
  } else {
      alert("No composition is active or no layer is selected!");
  }
};
